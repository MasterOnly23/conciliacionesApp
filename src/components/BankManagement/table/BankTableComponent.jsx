...
export const BankTableComponent = ({columns, data, title}) => {
      ...
      <thead>
        {headerGroups.map((headerGroup, i) => (
            ...
            {headerGroup.headers.map((column, j) => (
                <th {...column.getHeaderProps()} key={j} className="text-center rows-header">
            ...
            return (
              <tr {...row.getRowProps()} key={i} className="rows-cell">
                {row.cells.map((cell, j) => (
                  <td {...cell.getCellProps()} key={j} className="rows-body">
                    {cell.render("Cell")}
                  </td>
                ))}
              </tr>
